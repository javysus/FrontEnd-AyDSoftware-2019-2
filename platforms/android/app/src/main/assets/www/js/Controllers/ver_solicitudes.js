...
$.ajax({
  ...
  success: function(respuesta) {
    for (var i = respuesta.length - 1; i >= 0; i--){
      ...
      if(!respuesta[i].aceptada_fecha && respuesta[i].aceptada_cli){ /*Caso donde solo se acepto el precio, pero no la fecha. Enviada a administrador*/
      ...
      else if(respuesta[i].aceptadaAdm && respuesta[i].aceptada_cli){ /*Caso en el que solo queda aceptar o rechazar fecha*/
        $("#allSolicitudes").append('\
          <div class="card text-center mb-3" id=sol'+respuesta[i].id+'>\
          <div class="card-header">\
